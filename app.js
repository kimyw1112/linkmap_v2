...
(function lockDevTools(){
  ...
  /* 4) DevTools 열림 감지 → 화면 블러 (크기 차이 방식) */
  let _dtOpen=false;
  setInterval(()=>{
    const threshold=160;
...
const SENSITIVE_PATTERNS = [
  ...
  {
    re: /\d{2,3}[^\d]?\d{3,4}[^\d]?\d{4}/,
    msg: '전화번호 형식의 숫자는 입력할 수 없습니다',
  },
  {
...
function buildAlerts(threshold){
  const thr = threshold ?? alertThreshold;
